...
class OwnerControllerTest {
    ...
    void setUp() throws Exception {
        owners = new HashSet<>();
        LongStream.rangeClosed(123L, 130L).forEach(e -> {
    ...
    final void testFindOwners() throws Exception {
        mvc.perform(get("/owners/find")).andExpect(status().isOk()).andExpect(view().name("notimplemented"));
    ...
    final void testListOwners() throws Exception {
        ...
        mvc.perform(get("/owners")).andExpect(status().isOk()).andExpect(view().name("owners/index")).andExpect(model().attribute("owners", hasSize(8)));
    }
